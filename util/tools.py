...
def initialize_networks(args, action_space_size, dtype, device):
    ...
    if args.model_path is None:
        policy_network_base = Policy(args.hist_length, action_space_size, log_std=args.log_std)
        value_network = Value(args.hist_length)
    else:
        policy_network_base, value_network, _ = pickle.load(open(args.model_path, "rb"))
    policy_network = MyPolicy(policy_network_base)
    policy_network.to(device).to(dtype)
    value_network.to(device).to(dtype)
    # Unfreeze specific layers for training
    unfreeze_layers = ["linear1", "linear2", "resnet.classifier"]
    for name, para in policy_network.named_parameters():
...
def update_networks(batch: object, iteration: int, policy_network: MyPolicy, value_network: Value, optimizer_policy: torch.optim.Optimizer, optimizer_value: torch.optim.Optimizer, args: Namespace, dtype: torch.dtype, device: torch.device, optim_epochs: int = 10, optim_batch_size: int = 64) -> object:
    ...
    with torch.no_grad():
        ...
        fixed_log_probs = policy_network.get_log_prob(color_img, imgs_depth, goals, rays, hist_actions, actions)
    # Estimate advantages
    ...
    for _ in range(optim_epochs):
        ...
        for i in range(optim_iter_num):
            ...
            batch_data = iter_prepare([color_img, imgs_depth, goals, rays, hist_actions, actions, returns, advantages, fixed_log_probs], start_index, optim_batch_size)
            (color_img_b, imgs_depth_b, goals_b, rays_b, hist_actions_b, actions_b, returns_b, advantages_b, fixed_log_probs_b) = batch_data
            ppo_step(policy_network, value_network, optimizer_policy, optimizer_value, 1, color_img_b, imgs_depth_b, goals_b, rays_b, hist_actions_b, actions_b, returns_b, advantages_b, fixed_log_probs_b, args.clip_epsilon, args.l2_reg)
